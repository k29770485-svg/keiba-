...
`PredictionHistoryBoard.sample.tsx` は、通信状態を次のように明示します。

| 状態 | 表示 | データの扱い |
| --- | --- | --- |
| 初回読込 | スケルトンと「読み込んでいます」を表示 | まだ成功データがないため、表は表示しません。 |
| 再取得中 | ヘッダーの「更新中」と進行表示 | 前回成功した履歴を維持します。 |
| 初回取得失敗 | エラー詳細と「再試行」ボタンを表示 | 空の表や誤った結果は表示しません。 |
| 再取得失敗 | 警告と「前回取得分を表示しています」を表示 | 前回成功データを維持し、60秒後に自動再試行します。 |

手動の「今すぐ更新」または「再試行」ボタンは、進行中のリクエストを中断して新しいリクエストへ切り替えます。
